...
st.markdown("""
    ...
    /* Security: Remove the eye icon from API Key field */
...
# --- 3. STEPPER (Native Streamlit — as requested) ---
def render_stepper(current_step):
    """Renders progress using native columns to ensure zero rendering errors."""
        ...
        if st.session_state.connected:
            if st.button("Next Step: Create Blueprint ➡️"):
                if topic and age_group:
                    ...
                    # Initialize dataframe with editable columns
...
# --- 6. STEP 2: BLUEPRINT & PROMPTS ---
    ...
    st.markdown(f"**Topic:** {st.session_state.get('topic', '')} | **Age:** {st.session_state.get('age_group', '')}")
    ...
    # Generate and display prompts based on table input
    for index, row in st.session_state.df.iterrows():
        scene = row['Scene Description'] if row['Scene Description'] else st.session_state.topic
        full_prompt = (
            f"Coloring book page for {st.session_state.age_group} kids. "
            f"Subject: {scene}. Style: {st.session_state.style_tags}. "
            "Clean line art, vector style, no grayscale."
        )
        # Store back in DF for final step
        st.session_state.df.at[index, 'Prompt'] = full_prompt
        
        with st.expander(f"Page {row['Page']} AI Prompt"):
...
# --- 7. STEP 3: DONE ---
    ...
    st.subheader("Your Final Blueprint Summary")
    st.dataframe(st.session_state.df, use_container_width=True, hide_index=True)

    if st.button("🔄 Start New Project"):
        for key in ['step', 'connected', 'topic', 'age_group', 'style_tags', 'df', 'api_key']:
